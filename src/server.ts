...
function bootstrap() {
  try {
    server = app.listen(config.port, () => {
      logger.info(`🚀 WriteFlow AI backend operational on port ${config.port} in [${config.env}] mode.`);
    });
  } catch (err) {
...
process.on('unhandledRejection', (err) => {
  ...
  if (server) {
    server.close(() => {
      logger.warn('HTTP server terminated gracefully due to unhandled rejection.');
      process.exit(1);
    });
  } else {
    process.exit(1);
